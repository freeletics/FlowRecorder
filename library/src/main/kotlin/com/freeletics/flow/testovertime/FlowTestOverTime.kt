...
/**
 * Use [Observable.testOverTime] to create a new instance.
 *
 ...
 *
 * ```
 * val stateObserver = stateMachine.state.testOverTime()
 *
 * stateObserver shouldEmitNext State.Loading
 * stateObserver shouldEmitNext State.Content(list("FirstItem", "SecondItem"))
 * ```
 *
                ...
                verifiedHistory.size < replayValues.size -> {
                    ...
                    Assert.fail(
                        "Unverified items detected that you have never checked before by " +
                                "calling .shouldEmitNext(...)." +
                                "\nVerified: $verifiedHistory" +
                                "\nAll emitted items: $replayValues" +
                                "\nUnverified items emitted after last .shouldEmitNext() call: ${replayValues.subList(
                                    verifiedHistory.size, replayValues.size
                                )}"
                    )
                }
 ...
 * [FlowEmissionHistory.shouldNotEmitMoreValues] for additional assertions.
 *
 * @param timeoutConfig Defines the TimeoutConfig
 *
